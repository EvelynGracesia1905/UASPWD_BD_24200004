...
const server = http.createServer((req, res) => {
  ...
  if(req.method === 'GET'){
    ...
    req.on('end', () => {
      ...
      const {nama, email, item, duration, message} = parsed;


 

      const sql = 'insert into order (nama, email, item, duration, message) values(?, ?, ?, ?, ?)';

      db.query(sql, [nama, email, item, duration, message], (err) => {
